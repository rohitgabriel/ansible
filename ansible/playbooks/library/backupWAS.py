...
def main():
    ...
    if not os.path.exists(wasdir+"profiles/" + profilename):
        module.fail_json(msg=profilename+" path does no exist")


    # Run stopServer.sh on all the JVM's that are started
    if state == 'present':
        child = subprocess.Popen([wasdir+"profiles/" + profilename + "/bin/backupConfig.sh -profileName " + profilename + " " + backupdir+"backup-" + profilename + ".zip " + "-username " + username + " -password " + password], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout_value, stderr_value = child.communicate()
        if child.returncode != 0:
            module.fail_json(msg="profile backup failed", stdout=stdout_value, stderr=stderr_value)

        module.exit_json(changed=True, msg=name + " backup created successfully", stdout=stdout_value)
