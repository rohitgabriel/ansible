...
def main():
    ...
    # Run startServer.sh on all the JVM's that are started

    if state == 'present':
      child = subprocess.Popen([wasdir+"profiles/" + profilename + "/bin/startServer.sh " + jvmname ], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
      stdout_value, stderr_value = child.communicate()
      if child.returncode != 0:
        module.fail_json(msg="Unable to run startServer.sh successfully: "  , stdout=stdout_value, stderr=stderr_value)
