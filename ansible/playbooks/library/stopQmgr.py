...
def main():
    ...
    if state == 'present' and os.path.getsize(mqmfiledir) > 0:
      f = open(mqmfiledir)
      #with open('/tmp/list.log') as f:
      for line in f:
        child = subprocess.Popen(["endmqm " + line], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout_value, stderr_value = child.communicate()
        if child.returncode != 0:
            module.fail_json(msg="Unable to stop QMGR successfully: " + line, stdout=stdout_value, stderr=stderr_value)
    else:
        module.exit_json(changed=True, msg="All Qmgr's found stopped")
    module.exit_json(changed=True, msg=" ran endmqm successfully ", stdout=stdout_value)
